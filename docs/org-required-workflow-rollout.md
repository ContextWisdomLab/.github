...
- Merge posture: the default merge mode is `direct_or_auto`. When a current-head approved PR is same-repository and GitHub mergeability is `CLEAN`, the scheduler requests an immediate guarded squash merge with `--match-head-commit` before attempting a branch update. When the PR is approved but not yet clean, the scheduler may keep native auto-merge queued while waiting for branch freshness, checks, or GitHub mergeability to clear.
...
- OpenCode approval dispatches the merge scheduler with `pr_number` and `max_prs=1`, so the just-approved PR is inspected immediately instead of waiting for a broad queue scan to reach it.
