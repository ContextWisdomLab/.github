...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "timeout-minutes: 45" "strix workflow job budget covers PR-scoped Strix scans"
...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'FAILED_CHECK_EVIDENCE_ATTEMPTS: "10"' "opencode review workflow keeps pre-model peer-check waiting bounded for required workflow DX"
