...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'jq -r "$jq_filter" >"$output_file"' "opencode approval filters slurped commit check-runs with jq instead of unsupported gh api --jq"
