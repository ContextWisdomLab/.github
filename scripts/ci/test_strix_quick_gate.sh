...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" '[ "${check_lookup_token_source:-}" = "configured" ] && [ "${GH_REPOSITORY:-}" != "${GITHUB_REPOSITORY:-}" ]' "opencode approval treats configured cross-repository check lookup tokens as branch-protection delegated when statusCheckRollup is unavailable"
	assert_file_contains "$workflow_file" 'configured cross-repository token; branch protection remains authoritative' "opencode approval explains configured cross-repository check lookup limitations"
