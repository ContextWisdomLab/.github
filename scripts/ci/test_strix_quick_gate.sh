...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_not_contains "$workflow_file" 'workflow_call:' "scheduler stays materializable as an organization required workflow"
	...
	assert_file_contains "$workflow_file" 'workflows: ["OpenCode Review"]' "scheduler reruns after OpenCode Review completion so approvals can trigger merge/update actions"
	...
	assert_file_contains "$workflow_file" "github.event_name == 'pull_request_target' || github.event.inputs.trigger_reviews == 'true'" "scheduler enables review dispatch by default for required-workflow PR events"
	assert_file_contains "$workflow_file" "github.event_name == 'workflow_run' || github.event.inputs.enable_auto_merge == 'true'" "scheduler enables auto-merge after OpenCode Review completion"
	assert_file_contains "$workflow_file" "github.event_name == 'workflow_run' || github.event.inputs.update_branches == 'true'" "scheduler enables branch updates after OpenCode Review completion"
	...
	assert_file_contains "$workflow_file" 'github.workflow_sha' "scheduler checks out the exact required-workflow source commit instead of defaulting to main"
