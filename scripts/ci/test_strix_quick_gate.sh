...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "format('pr-{0}-{1}', inputs.pr_number || github.event.inputs.pr_number, inputs.pr_head_sha || github.event.inputs.pr_head_sha)" "strix workflow scopes manual or caller PR evidence concurrency to the requested pull request head"
	assert_file_contains "$workflow_file" "(inputs.pr_number || github.event.inputs.pr_number) != '' && format('pr-{0}', inputs.pr_number || github.event.inputs.pr_number)" "strix workflow retains a PR fallback group when no head SHA is provided"
	...
	assert_file_contains "$workflow_file" 'IS_PR_EVIDENCE_RUN: ${{ (github.event_name == '"'"'pull_request_target'"'"' || (inputs.pr_number || github.event.inputs.pr_number) != '"'"''"'"') && '"'"'true'"'"' || '"'"'false'"'"' }}' "strix workflow passes PR evidence mode through env"
	...
	assert_file_contains "$workflow_file" "inputs.strix_llm || github.event.inputs.strix_llm || 'openai/gpt-5'" "strix workflow defaults PR Strix scans to GitHub Models GPT-5"
...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'group: opencode-review-${{ github.event_name }}-${{ github.event.pull_request.number || inputs.pr_number || github.event.inputs.pr_number || github.run_id }}' "opencode review cancels stale runs per PR instead of preserving older review heads"
	...
	assert_file_contains "$workflow_file" "if: always() && (github.event_name == 'workflow_dispatch' || github.event_name == 'pull_request_target' || github.event_name == 'workflow_call')" "opencode review side effects are limited to manual, required PR, or reusable caller events"
	...
	assert_file_contains "$workflow_file" "github.event_name == 'workflow_dispatch' || github.event_name == 'pull_request_target' || github.event_name == 'workflow_call'" "manual, required, and reusable OpenCode reviews measure coverage instead of approving skipped coverage evidence"
	assert_file_contains "$workflow_file" 'ref: ${{ github.event.pull_request.head.sha || inputs.pr_head_sha || github.event.inputs.pr_head_sha }}' "coverage evidence checks out the requested PR head SHA as data"
	...
	assert_file_contains "$workflow_file" 'HEAD_SHA: ${{ github.event.pull_request.head.sha || inputs.pr_head_sha || github.event.inputs.pr_head_sha }}' "opencode evidence step passes HEAD_SHA to failed-check evidence collection"
...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_contains "$REPO_ROOT/.github/workflows/opencode-review.yml" "github.event_name == 'workflow_call'" "opencode review job accepts reusable workflow callers"
