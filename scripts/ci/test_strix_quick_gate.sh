...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "600"' "opencode primary review gives each model attempt enough time before fallback"
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "3600"' "opencode model pool has a merge-safe total retry budget"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "2"' "opencode GPT-5 fallback uses one bounded attempt before trying the catalog pool"
	...
	assert_file_contains "$workflow_file" "failed closed without posting a deterministic REQUEST_CHANGES review" "opencode approval avoids deterministic model-exhaustion review comments"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "2"' "opencode catalog fallback retries each model with bounded exponential backoff"
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "600"' "opencode catalog fallback gives each bounded model attempt enough runtime"
