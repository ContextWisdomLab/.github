...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "15"' "opencode fallback now retries each model with bounded exponential backoff before moving to the next candidate"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 330' "opencode approval step has a bounded wall-clock timeout"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "15"' "opencode primary and fallback paths allow bounded multi-attempt retries for transient model failures"
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "15"' "opencode catalog fallback retries each model with bounded exponential backoff before moving on"
