...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/collect_failed_check_evidence.sh" 'seen[$5] = 1' "failed-check evidence records seen workflow run ids while merging rollup and workflow evidence"
	assert_file_contains "$REPO_ROOT/scripts/ci/collect_failed_check_evidence.sh" '"$failed_contexts" "$workflow_run_contexts"' "failed-check evidence avoids duplicate workflow-run evidence when statusCheckRollup already includes the run"
