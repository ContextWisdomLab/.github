...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "leave the PR review unchanged for orchestration outcomes such as cancelled or skipped coverage-evidence jobs" "opencode approval does not turn orchestration-only coverage states into source review findings"
	...
	assert_file_contains "$workflow_file" "JavaScript/TypeScript coverage threshold" "opencode coverage evidence reports JS coverage threshold proof separately"
