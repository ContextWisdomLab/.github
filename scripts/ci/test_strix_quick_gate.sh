...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "2"' "opencode fallback retries each model with exponential backoff before moving to the next candidate"
	...
	assert_file_contains "$workflow_file" "github-models/openai/gpt-5-mini github-models/openai/gpt-5-nano github-models/openai/o4-mini" "opencode review keeps a small OpenAI fallback pool after nano-only review exhaustion"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "2"' "opencode primary and fallback paths allow bounded per-model exponential retries"
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "2"' "opencode catalog fallback uses two total attempts per model before moving on"
	...
	assert_file_contains "$workflow_file" "github-models/openai/gpt-5-mini github-models/openai/gpt-5-nano github-models/openai/o4-mini" "opencode review includes additional OpenAI reasoning model fallbacks"
