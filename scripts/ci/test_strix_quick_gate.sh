...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "1200"' "opencode model pool has a bounded twenty-minute total retry budget"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "12"' "opencode fallback keeps retrying within the bounded model-pool budget"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "12"' "opencode primary and fallback paths keep retrying each model within a bounded budget"
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "12"' "opencode catalog fallback retries each model multiple times before budget exhaustion"
