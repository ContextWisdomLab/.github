...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" '(.name // "") == "scan-pr-queue" and ((.checkSuite.workflowRun.workflow.name // "") == "PR Review Merge Scheduler" or (.checkSuite.workflowRun.workflow.name // "") == "Required PR Review Merge Scheduler")' "opencode approval ignores cancelled scheduler queue replacement checks without source evidence"
	...
	assert_file_contains "$workflow_file" 'fail_review_publication_failure()' "opencode approval hard-fails pull review publication errors"
	assert_file_contains "$workflow_file" 'OpenCode could not publish %s; failing because required approval reviews must be durable.' "opencode approval refuses to pass without a durable PR review"
	...
	assert_file_contains "$workflow_file" 'fail_review_publication_failure "pull review"' "opencode approval hard-fails permission-denied review publication"
