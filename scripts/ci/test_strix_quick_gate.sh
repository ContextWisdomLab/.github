...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_not_contains "$workflow_file" "--cov-fail-under=100" "opencode coverage evidence does not hard-fail PRs solely below 100 percent test coverage"
	assert_file_contains "$workflow_file" "Coverage below 100% (informational):" "opencode coverage evidence reports below-100 coverage as review context"
	...
	assert_file_contains "$workflow_file" "Coverage and Docstring coverage labels must cite Coverage execution evidence when coverage measurement ran" "opencode approval requires measured coverage evidence without hard-gating below-100 coverage"
	assert_file_contains "$workflow_file" "including measured percentages or not-applicable evidence" "opencode approval permits evidence-backed measured or no-source coverage"
