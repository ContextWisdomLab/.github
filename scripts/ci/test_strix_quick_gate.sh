...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "16800"' "opencode model pool retry budget leaves safety margin under the 300-minute bounded step timeout"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "4"' "opencode fallback gives each model multiple bounded retries before moving on"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "4"' "opencode model pool uses bounded multi-attempt retries per model before moving on"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 300' "opencode model pool step allows very large repository reviews before falling back"
