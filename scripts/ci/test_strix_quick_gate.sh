...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "4"' "opencode primary and fallback paths use bounded multi-attempt retries per model"
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_ATTEMPTS: "4"' "opencode catalog fallback retries each model with exponential backoff before moving on"
