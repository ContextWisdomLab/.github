#!/usr/bin/env python3
"""Submit a second, independent PR approval after primary review gates pass."""
...
def approve(repo: str, number: int, pr: dict[str, Any], actor: str) -> None:
    ...
    body = "\n".join(
        [
            ...
            "",
            "The secondary review gate found a current-head primary OpenCode approval, no current-head change requests, no unresolved review threads, and no blocking GitHub checks.",
            "",
            "- Result: APPROVE",
            f"- Head SHA: `{head_sha}`",
