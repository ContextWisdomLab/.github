#!/usr/bin/env python3
"""Submit a deterministic, non-OpenCode PR approval after primary gates pass."""
...
def approve(repo: str, number: int, pr: dict[str, Any], actor: str) -> None:
    head_sha = str(pr.get("headRefOid") or "")
    source = os.environ.get("SECONDARY_REVIEW_TOKEN_SOURCE") or "SECONDARY_REVIEW_TOKEN"
    body = "\n".join(
        [
            "## Secondary review gate",
            "",
            "The deterministic secondary review program found a current-head primary OpenCode approval, no current-head change requests, no unresolved review threads, and no blocking GitHub checks.",
            "",
            "This approval is generated by the deterministic secondary gate, not by OpenCode model review.",
            "",
            "- Result: APPROVE",
            f"- Head SHA: `{head_sha}`",
            ...
            "",
            f"<!-- secondary-review-gate head_sha={head_sha} -->",
        ]
    )
    payload = {
        "commit_id": head_sha,
    ...
    print(f"Secondary approval submitted for {repo}#{number} at {head_sha}.")
...
def inspect_and_approve(repo: str, number: int) -> int:
    ...
    if existing_secondary_review(pr, actor):
        print("Current head already has a secondary approval; nothing to do.")
        return 0
    if not current_primary_approval(pr):
        print("Current head does not have a primary OpenCode approval; secondary approval skipped.")
        return 0
    if has_current_changes_requested(pr):
        print("Current head has requested changes; secondary approval skipped.")
        return 0
    if has_unresolved_threads(pr):
        print("PR has unresolved review threads; secondary approval skipped.")
        return 0
    blockers = blocking_checks(pr)
    if blockers:
        print("Blocking checks remain; secondary approval skipped:")
