...
def iter_json_objects(text: str) -> list[Any]:
    ...
    while True:
        ...
        if index == -1:
            break
        # When `raw_decode` fails, it counts newlines from the start of the string
        # to format the error message. Passing the `index` parameter causes O(N)
        # behavior per failure because it scans the entire prefix for newlines.
        # Slicing the string `text[index:]` is faster because the prefix is dropped
        # and the error formatter only scans the remaining string.
        try:
