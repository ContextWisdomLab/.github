...
def get_current_head_opencode_review_state(pr: dict[str, Any]) -> str | None:
    """Return the latest actionable OpenCode review state for the current head, or None."""
    for review in reversed((pr.get("reviews") or {}).get("nodes") or []):
        ...
        if state == "APPROVED" and is_deterministic_fallback_approval(review):
            return None
...
def inspect_pr(
    ...
    *,
    ...
    current_head_approved = head_review_state == "APPROVED"
