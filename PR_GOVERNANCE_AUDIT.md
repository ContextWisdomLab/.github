...
| `scopeweave` | PR #127 is the current representative trace. Dry-run `28147098767` selected `auto_merge`, but live run `28147157319` failed with `GraphQL: Resource not accessible by integration (mergePullRequest)` because the repo-local scheduler token had `contents: read`. Commit `6601953` changes only the scheduler workflow to `contents: write` and adds a self-test assertion while keeping OpenCode read-only. |
...
| `scopeweave` | Strix self-test and the central scheduler are useful rollout fixtures. Live scheduler run `28147157319` proved `action_error` is reported per PR instead of aborting the queue. | The scheduler could identify #127 as merge-ready, but its default workflow token lacked `contents: write`, so GitHub Actions could not execute the merge mutation. | Keep it on the central path, give only the scheduler `contents: write`, keep OpenCode read-only, and require a new-head approval/check rerun before retrying merge. |
